...
def plot_em_trend(df, grade, limits):
    ...
    ax.plot(df_f["日期"], df_f["浮游菌(cfu/m³)"], marker='o', linestyle='-', color='royalblue', label='监测值')
    ax.axhline(y=alert, color='orange', linestyle='--', label=f'警戒限 {alert}')
    ax.axhline(y=action, color='red', linestyle='--', label=f'纠偏限 {action}')
    ax.set_title(f"{grade} 浮游菌趋势")
    ax.set_xlabel("日期")
    ax.set_ylabel("cfu/m³")
    ax.legend()
...
def plot_endotoxin_curve(df):
    ...
    ax.scatter(x, y, color='red', s=80, label='实测值')
    ax.plot(x_range, y_range, color='green', linewidth=2, label=f'拟合线 (R²={r2:.4f})')
    ax.set_title(f"内毒素标准曲线 (Y = {slope:.3f}X + {intercept:.3f})")
    ax.set_xlabel("浓度 (EU/mL)")
    ax.set_ylabel("OD值 (405nm)")
    ax.legend()
    plt.tight_layout()
